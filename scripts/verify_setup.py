...
def main():
    ...
    for mod in ("fastapi", "uvicorn", "numpy", "librosa", "xgboost", "joblib",
                 "pandas", "parselmouth", "soundfile"):
        try:
            __import__(mod)
            check(mod, True)
        except ImportError:
    ...
    check(".env file exists", has_env, "Copy .env.example to .env and set GEMINI_API_KEY")
    if has_env:
    ...
    check(f"Depression model ({dep_model.name})", dep_model.exists(),
          "Copy phq_xgb.pkl from depression-ai-interviewer")
    anx_model = BASE / "models" / "anxiety" / "gbr_pipeline.joblib"
    check(f"Anxiety model ({anx_model.name})", anx_model.exists(),
          "Copy gbr_pipeline.joblib from anxiety-ai-interviewer")

    # 5. HF cache (SER)
    print("\n5. HuggingFace model cache")
    try:
        from huggingface_hub import try_to_load_from_cache
        ser_cached = try_to_load_from_cache(
            "jonatasgrosman/wav2vec2-large-xlsr-53-english", "model.safetensors"
        )
        check("Depression SER model cached", ser_cached is not None and isinstance(ser_cached, str),
    ...
    try:
        from huggingface_hub import try_to_load_from_cache
        stress_cached = try_to_load_from_cache(
            "forwarder1121/voice-based-stress-recognition", "config.json"
        )
        check("Stress StudentNet cached", stress_cached is not None and isinstance(stress_cached, str),
    ...
    check("ffmpeg in PATH", ffmpeg is not None,
